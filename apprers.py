...
# Adding mapping for party names
party_map = {
...
df_hist['margin%'] = pd.to_numeric(df_hist['margin%'].astype(str).str.replace('%', '', regex=False), errors='coerce')
...
app.layout = html.Div(style={'fontFamily': 'Arial'}, children=[
    ...
    dcc.Tabs([
        dcc.Tab(label='* Constituency Map *', children=[
            html.Div([
                html.Label("Select Year:", style={'padding': '10px', 'fontWeight': 'bold'}),
                dcc.Dropdown(
                    id='year-dropdown',
                    options=[{'label': str(int(year)), 'value': year} for year in available_years],
        ...
        dcc.Tab(label='* Margin% Map *', children=[
            html.Div([
                html.Label("Select Year:", style={'padding': '10px', 'fontWeight': 'bold'}),
                dcc.Dropdown(
        ...
        dcc.Tab(label='* Repeat Winners Sunburst *', children=[
...
def plot_map(selected_year):
    ...
    df_year = df_hist[df_hist['year'] == selected_year][['Pc_name', 'party', 'candidate_name']].drop_duplicates()
    ...
    color_map = {
        ...
        'AAP': '#00CED1', 'CPM': '#FF0000', 'TMC': '#00FF00',
        'JDU': '#FFFF00', 'SP': '#800080', 'OTHER': '#808080'
    }
...
# Processing for Risk/Opportunity Map
# Setup bins for margin %
...
@app.callback(
    Output('margin-map-graph', 'figure'),
    Input('margin-year-dropdown', 'value')
)
def plot_margin_map(selected_year):
    df_year = df_hist[df_hist['year'] == selected_year][['Pc_name', 'margin%', 'margin_color', 'party']].drop_duplicates()
    merged_map = geo.merge(df_year, left_on='pc_name', right_on='Pc_name', how='left')
    merged_map['margin_color'] = merged_map['margin_color'].fillna('gray')  # For missing data
    ...
    fig = px.choropleth(
        ...
        hover_name="pc_name",
        hover_data='party',
        title=f"Victory Margin% by Constituency(Risk/Opportunity Analysis) ({int(selected_year)})",
...
candidate_wins = df_hist.groupby(['candidate_name', 'party']).size().reset_index(name='wins')
...
def plot_sunburst(_):
    ...
    fig = px.sunburst(
        sunburst_data,
        path=['party', 'wins', 'candidate_name'],
        values='wins',
        color='party',
        title='Candidates Who Won 5 Or More Times (by Party)[The Pro Players]',
        height=1300,
        width=1300
    )
    fig.update_layout(
        margin=dict(t=50, l=450, r=0, b=0),
        font=dict(size=14)
    )
    return fig
